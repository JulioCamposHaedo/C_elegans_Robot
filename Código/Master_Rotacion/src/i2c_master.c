...
// Lista de eventos de callback del I2C
typedef enum {
    I2C_DATA_COMPLETE = 0,
    I2C_WRITE_COLLISION,
    I2C_ADDR_NACK,
...
// Estructura estado I2C
typedef struct
{
    i2c_callback_t callbackTable[6];
    void *callbackPayload[6];           //  cada callback puede tener un payload
    uint16_t time_out;                  // Contador del Timeout I2C entre eventos I2C
    uint16_t time_out_value;            // Recarga valores para los timeouts
    i2c_address_t address;             // El address I2C
    uint8_t *data_ptr;                  // Puntero a bufer de datos
    size_t data_length;                 // Bytes en el bufer
    i2c_fsm_states_t state;            // Estado del Driver
    i2c_error_t error;
...
/* Interfaz de interrupciones Interrupt */
...
i2c_error_t I2C_Open(i2c_address_t address)
{
    ...
    if(!I2C_Status.inUse)
    {
        ...
        // Setea todos los callbacks a un  stop
